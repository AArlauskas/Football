...
public interface GuessRepository extends JpaRepository<Guess, Long> {
    @Transactional
    @Modifying
    @Query(value = "insert into guess (game_id, user_id) select g.id, u.id from game g cross join user u where g.closed is not null and not exists (select 1 from guess gs where gs.game_id=g.id and gs.user_id=u.id)", nativeQuery = true)
